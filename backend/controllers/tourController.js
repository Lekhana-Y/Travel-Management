...
export const getTourBySearch = async (req, res) => {
  ...
  try {
    ...
    if (!distance && !maxGroupSize) {
      ...
      tours = await Tour.find({ city }).populate("reviews");
    } else if (distance && maxGroupSize) {
      // All fields provided — apply < distance filtering
      ...
      // Any other combination (e.g., city + distance only OR city + size only)
      const query = { city };
      if (distance) query.distance = { $lte: distance };
      if (maxGroupSize) query.maxGroupSize = { $gte: maxGroupSize };

      tours = await Tour.find(query).populate("reviews");
    }
